...
class OdomTfPublisher():
    ...
    def __init__(self):
        ...
        self.tf.header.frame_id = rospy.get_param('~init_source_frame_name', 'odom')
        self.tf.child_frame_id = rospy.get_param('~target_frame_name', 'base_link')
        tf_init = rospy.get_param('~init_tf_pose', [0., 0., 0., 0., 0., 0.])
        ...
        sub_topic = rospy.get_param('updater_topic', '')
        self.subscription = None
        if sub_topic:
            # Subscribe to a turtle{1}{2}/pose topic and call handle_turtle_pose
            # callback function on each message
            self.subscription = rospy.Subscriber(sub_topic, Odometry, self.handle_tf_update)

        # Initialize the transform broadcaster
        self.tf_broadcaster = TransformBroadcaster()
        # set a timer callback to guard the minimum update frequency.
        tf_freq = rospy.get_param('~min_tf_broadcast_frequency', 10.0)
        self.send_tf_timer = rospy.Timer(rospy.Duration(1./tf_freq), self.send_tf_callback)


    def send_tf_callback(self, event=None):
...
def main():
    rospy.init_node("odom_tf_publisher_node")
